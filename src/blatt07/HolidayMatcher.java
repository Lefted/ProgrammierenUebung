...
class HolidayFeatures {
	...
	void setFeature(Features feature) {
		...
		// shift bit to the index
		long bitForFeature = 0b1 << index;
		// or mask in into the featureBit attribute
		featureBits |= bitForFeature;
	}
	...
	public int match(HolidayFeatures toMatch) {
		// TODO
		long sameFeaturesBits = toMatch.featureBits & featureBits;
		return Long.bitCount(sameFeaturesBits);
	}
	...
	public static void main(String[] args) {
		...
		HolidayFeatures hf3 = new HolidayFeatures();
		hf3.random();
		System.out.println(hf3.toString());
	}
}
...
class Customer {
	...
	void printBestMatches(Hotel hotels[]) {
		...
		// TODO
		for (Hotel hotel : hotels) {
			if (hotel.features.match(features) == bestMatchCount) {
				System.out.println(String.format("%s: %s", hotel.name, hotel.features));
			}
		}
	}
}
